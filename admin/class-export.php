...
class WPSEO_Export {
	...
	public function get_results() {
		...
		if ( $this->success ) {
			...
			$results['msg']    = wp_kses_post(sprintf( __( 'Export created: %1$sdownload your export file here%2$s.', 'wordpress-seo' ), '<a href="' . $this->export_zip_url . '">', '</a>' ));
		}
		else {
			$results['status'] = 'failure';
			/* translators: %1$s expands to Yoast SEO */
			$results['msg']    = wp_kses_post(sprintf( __( 'Error creating %1$s export: ', 'wordpress-seo' ), 'Yoast SEO' ) . $this->error);
		}
	...
	private function export_settings() {
		...
		if ( $this->write_file() ) {
			if ( $this->zip_file() ) {
				...
				$this->error = esc_html__( 'Could not zip settings-file.', 'wordpress-seo' );

				return false;
			}
		}
		$this->error = esc_html__( 'Could not write settings to file.', 'wordpress-seo' );
	...
	private function export_header() {
		/* translators: %1$s expands to Yoast SEO */
		$this->write_line( '; ' . sprintf( esc_html__( 'This is a settings export file for the %1$s plugin by Yoast.com', 'wordpress-seo' ), 'Yoast SEO' ) . ' - https://yoast.com/wordpress/plugins/seo/' );
		if ( $this->include_taxonomy ) {
			$this->write_line( '; ' . esc_html__( 'This export includes taxonomy metadata', 'wordpress-seo' ) );
		}
	}
	...
	private function taxonomy_metadata() {
		if ( $this->include_taxonomy ) {
			...
			if ( is_array( $taxonomy_meta ) ) {
				...
				$this->write_line( '; ' . esc_html__( 'No taxonomy metadata found', 'wordpress-seo' ), true );
			}
		}
	}
