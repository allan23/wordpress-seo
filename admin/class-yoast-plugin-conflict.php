...
class Yoast_Plugin_Conflict {
	...
	protected function set_error( $plugin_section, $readable_plugin_section ) {
		...
		$error_message     = '<h3>' . esc_html__( 'Warning!', 'wordpress-seo' ) . '</h3>';

		/* translators: %1$s: 'Facebook & Open Graph' plugin name(s) of possibly conflicting plugin(s), %2$s to Yoast SEO */
		$error_message .= '<p>' . wp_kses_post(sprintf( __( 'The %1$s plugin(s) might cause issues when used in conjunction with %2$s.', 'wordpress-seo' ), $plugins_as_string, 'Yoast SEO' )) . '</p>';
		$error_message .= sprintf( $readable_plugin_section, 'Yoast SEO', $plugins_as_string ) . '<br/><br/>';
		$error_message .= '<p><strong>' . esc_html__( 'Recommended solution', 'wordpress-seo' ) . '</strong><br/>';

		/* translators: %1$s: 'Facebook & Open Graph' plugin name(s) of possibly conflicting plugin(s). %2$s to Yoast SEO */
		$error_message .= wp_kses_post(sprintf( __( 'We recommend you deactivate %1$s and have another look at your %2$s configuration using the button above.', 'wordpress-seo' ), $plugins_as_string, 'Yoast SEO' )) . '</p>';
		foreach ( $this->active_plugins[ $plugin_section ] as $plugin_file ) {

			/* translators: %s: 'Facebook' plugin name of possibly conflicting plugin */
			$error_message .= '<a target="_blank" class="button-primary" href="' . wp_nonce_url( 'plugins.php?action=deactivate&amp;plugin=' . $plugin_file . '&amp;plugin_status=all', 'deactivate-plugin_' . $plugin_file ) . '">' . wp_kses_post(sprintf( __( 'Deactivate %s', 'wordpress-seo' ), WPSEO_Utils::get_plugin_name( $plugin_file ) )) . '</a> ';
		}

		/* translators: %1$s expands to Yoast SEO */
		$error_message .= '<p class="alignright"><small>' . sprintf( esc_html__( 'This warning is generated by %1$s.', 'wordpress-seo' ), 'Yoast SEO' ) . '</small></p><div class="clear"></div>';
