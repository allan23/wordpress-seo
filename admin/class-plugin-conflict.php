...
class WPSEO_Plugin_Conflict extends Yoast_Plugin_Conflict {
	...
	public static function hook_check_for_plugin_conflicts( $plugin = false ) {
		...
		$plugin_sections = array(
			...
			'open_graph'   => esc_html__( 'Both %1$s and %2$s create OpenGraph output, which might make Facebook, Twitter, LinkedIn and other social networks use the wrong texts and images when your pages are being shared.', 'wordpress-seo' )
				. '<br/><br/>'
				. '<a target="_blank" class="button" href="' . admin_url( 'admin.php?page=wpseo_social#top#facebook' ) . '">'
				/* translators: %1$s expands to Yoast SEO */
				. sprintf( esc_html__( 'Configure %1$s\'s OpenGraph settings', 'wordpress-seo' ), 'Yoast SEO' )
				. '</a>',
			...
			'xml_sitemaps' => esc_html__( 'Both %1$s and %2$s can create XML sitemaps. Having two XML sitemaps is not beneficial for search engines, yet might slow down your site.', 'wordpress-seo' )
				. '<br/><br/>'
				. '<a target="_blank" class="button" href="' . admin_url( 'admin.php?page=wpseo_xml' ) . '">'
				/* translators: %1$s expands to Yoast SEO */
				. sprintf( esc_html__( 'Configure %1$s\'s XML Sitemap settings', 'wordpress-seo' ), 'Yoast SEO' )
				. '</a>',
		);
