...
class WPSEO_Import_AIOSEO extends WPSEO_Import_External {
	...
	private function import_ga() {
		if ( isset( $this->aioseo_options['aiosp_google_analytics_id'] ) ) {
			...
			$this->set_msg( wp_kses_post(__( sprintf(
			...
			$this->set_msg( wp_kses_post(__( sprintf( 'All in One SEO data successfully imported. Would you like to %sdisable the All in One SEO plugin%s.', '<a href="' . esc_url( admin_url( 'admin.php?page=wpseo_tools&tool=import-export&deactivate_aioseo=1#top#import-seo' ) ) . '">', '</a>' ), 'wordpress-seo' ) ));
		}
	}
