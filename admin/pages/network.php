...
if ( $use_dropdown === true ) {
	...
	$yform->textinput( 'defaultblog', esc_html__( 'New sites in the network inherit their SEO settings from this site', 'wordpress-seo' ), 'wpseo_ms' );
	echo '<p>' . sprintf( esc_html__( 'Enter the %sSite ID%s for the site whose settings you want to use as default for all sites that are added to your network. Leave empty for none (i.e. the normal plugin defaults will be used).', 'wordpress-seo' ), '<a href="' . esc_url( network_admin_url( 'sites.php' ) ) . '">', '</a>' ) . '</p>';
}
...
echo '<input type="submit" name="wpseo_submit" class="button-primary" value="' . esc_html__( 'Save MultiSite Settings', 'wordpress-seo' ) . '"/>';
echo '</form>';
...
if ( $use_dropdown === true ) {
	...
	$yform->select(
		'restoreblog',
		esc_html__( 'Site ID', 'wordpress-seo' ),
		$dropdown_input,
		'wpseo_ms'
	);
}
else {
	$yform->textinput( 'restoreblog', esc_html__( 'Blog ID', 'wordpress-seo' ), 'wpseo_ms' );
}

echo '<input type="submit" name="wpseo_restore_blog" value="' . esc_html__( 'Restore site to defaults', 'wordpress-seo' ) . '" class="button"/>';
echo '</form>';
