...
?>
<div id='redirect-<?php echo esc_attr(md5( $url )); ?>' style='display: none;'>
	<form>
		<div class='form-wrap wpseo_content_wrapper'>
		<?php
		switch ( $view_type ) {
			case 'create' :
				echo '<h3>', esc_html__( 'Redirect this broken URL and fix the error', 'wordpress-seo' ), '</h3>';
				?>
				<div class='form-field form-required'>
					<label for='wpseo-current-url'><?php esc_html_e( 'Current URL:', 'wordpress-seo' ); ?></label>
					<input type='text' id='wpseo-current-url' name='current_url' value='<?php echo esc_attr($url); ?>' disabled='disabled'>
				</div>
				<div class='form-field form-required'>
					<label for='wpseo-new-url'><?php esc_html_e( 'New URL:', 'wordpress-seo' ); ?></label>
					<input type='text' id='wpseo-new-url' name='new_url' autofocus value=''>
				</div>
				<div class='form-field form-required'>
					<label for='wpseo-mark-as-fixed' class='clear'><?php esc_html_e( 'Mark as fixed:', 'wordpress-seo' ); ?></label>
				...
				<p class='submit'>
					<input type='button' name='submit' id='submit' class='button button-primary' value='<?php esc_attr_e( 'Create redirect', 'wordpress-seo' ); ?>' onclick='wpseo_gsc_post_redirect( jQuery( this ) );' />
				</p>
				<?php
				break;

			case 'already_exists' :
				echo '<h3>', esc_html__( 'Error: a redirect for this URL already exists', 'wordpress-seo' ), '</h3>';
				echo '<p>';
			...
			case 'no_premium' :
				...
				echo '<h3>', sprintf( esc_html__( 'Creating redirects is a %s feature', 'wordpress-seo' ), 'Yoast SEO Premium' ), '</h3>';
				echo '<p>';
