...
if ( ! file_exists( $robots_file ) ) {
	if ( is_writable( get_home_path() ) ) {
		...
		echo '<input type="submit" class="button" name="create_robots" value="', esc_html__( 'Create robots.txt file', 'wordpress-seo' ), '">';
		echo '</form>';
	}
	else {
	...
	if ( ! is_writable( $robots_file ) ) {
		...
		echo '<p>', esc_html__( 'Edit the content of your robots.txt:', 'wordpress-seo' ), '</p>';
		echo '<textarea class="large-text code" rows="15" name="robotsnew">', esc_textarea($robots_txt_content), '</textarea><br/>';
		echo '<div class="submit"><input class="button" type="submit" name="submitrobots" value="', esc_html__( 'Save changes to Robots.txt', 'wordpress-seo' ), '" /></div>';
		echo '</form>';
	}
}
if ( ( isset( $_SERVER['SERVER_SOFTWARE'] ) && stristr( $_SERVER['SERVER_SOFTWARE'], 'nginx' ) === false ) ) {
	...
	if ( file_exists( $ht_access_file ) ) {
		...
		if ( ! is_writable( $ht_access_file ) ) {
			...
			echo '<p>', esc_html__( 'Edit the content of your .htaccess:', 'wordpress-seo' ), '</p>';
			echo '<textarea class="large-text code" rows="15" name="htaccessnew">', esc_textarea($contentht), '</textarea><br/>';
			echo '<div class="submit"><input class="button" type="submit" name="submithtaccess" value="', esc_html__( 'Save changes to .htaccess', 'wordpress-seo' ), '" /></div>';
			echo '</form>';
		}
	}
	else {
