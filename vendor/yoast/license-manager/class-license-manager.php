	...
	abstract class Yoast_License_Manager implements iYoast_License_Manager {
		...
		public function display_admin_notices() {
			...
			if ( ! $this->license_is_valid() ) {
				...
				?>
				<div class="error">
					<p><?php echo wp_kses_post(sprintf( __( $message, $this->product->get_text_domain() ), $this->product->get_item_name(), $this->product->get_license_page_url(), $this->product->get_tracking_url( 'activate-license-notice' ) )); ?></p>
				</div>
			<?php
			}
			...
			if ( defined( "WP_HTTP_BLOCK_EXTERNAL" ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
				...
				if ( ! defined( "WP_ACCESSIBLE_HOSTS" ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
					?>
					<div class="error">
						<p><?php echo wp_kses_post(sprintf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', $this->product->get_text_domain() ), $this->product->get_item_name(), '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>' )); ?></p>
					</div>
				<?php
				}
		...
		public function activate_license() {
			...
			if ( $result ) {
				...
				if ( $result->license === 'valid' ) {
					...
					$message = sprintf( esc_html__( "Your %s license has been activated. ", $this->product->get_text_domain() ), esc_html($this->product->get_item_name()) );

					// show a custom notice if users have an unlimited license
					if ( $result->license_limit == 0 ) {
						$message .= esc_html__( "You have an unlimited license. ", $this->product->get_text_domain() );
					} else {
						$message .= sprintf( esc_html__( "You have used %d/%d activations. ", $this->product->get_text_domain() ), $result->site_count, $result->license_limit );
					}
					...
					if ( $result->license_limit > 0 && ( $result->license_limit - $result->site_count ) <= 3 ) {
						$message .= wp_kses_post(sprintf( __( '<a href="%s">Did you know you can upgrade your license?</a>', $this->product->get_text_domain() ), $this->product->get_tracking_url( 'license-nearing-limit-notice' ) ));
						...
						$message .= wp_kses_post(sprintf( __( '<a href="%s">Your license is expiring in %d days, would you like to extend it?</a>', $this->product->get_text_domain() ), $this->product->get_tracking_url( 'license-expiring-notice' ), $days_left ));
					}
					...
					if ( isset( $result->error ) && $result->error === 'no_activations_left' ) {
						...
						$this->set_notice( wp_kses_post(sprintf( __( 'You\'ve reached your activation limit. You must <a href="%s">upgrade your license</a> to use it on this site.', $this->product->get_text_domain() ), $this->product->get_tracking_url( 'license-at-limit-notice' ) )), false );
					} elseif ( isset( $result->error ) && $result->error == "expired" ) {
						// show notice if the license is expired
						$this->set_notice( wp_kses_post(sprintf( __( 'Your license has expired. You must <a href="%s">extend your license</a> in order to use it again.', $this->product->get_text_domain() ), $this->product->get_tracking_url( 'license-expired-notice' ) )), false );
					} else {
						// show a general notice if it's any other error
						$this->set_notice( esc_html__( "Failed to activate your license, your license key seems to be invalid.", $this->product->get_text_domain() ), false );
					}
		...
		public function deactivate_license() {
			...
			if ( $result ) {
				...
				if ( $result->license === 'deactivated' ) {
					$this->set_notice( wp_kses_post(sprintf( __( "Your %s license has been deactivated.", $this->product->get_text_domain() ), $this->product->get_item_name() )) );
				} else {
					$this->set_notice( wp_kses_post(sprintf( __( "Failed to deactivate your %s license.", $this->product->get_text_domain() ), $this->product->get_item_name() )), false );
				}
		...
		protected function call_license_api( $action ) {
			...
			if ( $request->is_valid() !== true ) {
				$this->set_notice( wp_kses_post(sprintf( __( "Request error: \"%s\" (%scommon license notices%s)", $this->product->get_text_domain() ), $request->get_error_message(), '<a href="http://kb.yoast.com/article/13-license-activation-notices">', '</a>' )), false );
			}
		...
		public function show_license_form_heading() {
			?>
			<h3>
				<?php echo wp_kses_post(sprintf( __( "%s: License Settings", $this->product->get_text_domain() ), $this->product->get_item_name() )); ?>&nbsp; &nbsp;
			</h3>
		<?php
		}
