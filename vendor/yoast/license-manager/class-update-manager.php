...
if( ! class_exists( "Yoast_Update_Manager", false ) ) {
	...
	class Yoast_Update_Manager {
		...
		public function show_update_error() {
			...
			?>
			<div class="error">
				<p><?php echo wp_kses_post(sprintf( __( '%s failed to check for updates because of the following error: <em>%s</em>', $this->product->get_text_domain() ), $this->product->get_item_name(), $this->error_message )); ?></p>
			</div>
			<?php
			}
		...
		private function call_remote_api() {
			...
			if( isset( $response->license_check ) && ! empty( $response->license_check ) && $response->license_check != 'valid' ) {
				...
				$this->error_message = esc_html__( "This site has not been activated properly on yoast.com and thus cannot check for future updates. Please activate your site with a valid license key.", $this->product->get_text_domain() );
				add_action( 'admin_notices', array( $this, 'show_update_error' ) );
			}
