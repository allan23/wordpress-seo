...
if( $api_host_available['availability'] === false ){
	echo '<p style="color: red; max-width: 600px;"><strong>' . wp_kses_post(sprintf( __( 'We couldn\'t create a connection to our API to verify your license key(s). Please ask your hosting company to allow outgoing connections from your server to %s.', $product->get_text_domain() ), $api_host_available['url'] )) . '</strong></p>';
}

if( $api_host_available['curl_version'] !== false && version_compare( $api_host_available['curl_version'], '7.20.0', '<')){
	echo '<p style="color: red; max-width: 600px;"><strong>' . wp_kses_post(sprintf( __( 'Your server has an outdated version of the PHP module cURL (Version: %s). Please ask your hosting company to update this to a recent version of cURL. You can read more about that in our %sKnowledge base%s.', $product->get_text_domain() ), $api_host_available['curl_version'], '<a href="http://kb.yoast.com/article/90-is-my-curl-up-to-date" target="_blank">', '</a>' )) . '</strong></p>';
}
...
<table class="form-table yoast-license-form">
	<tbody>
		<tr valign="top">
			<th scope="row" valign="top"><?php esc_html_e( 'License status', $product->get_text_domain() ); ?></th>
			<td>
		...
		<tr valign="top">
			<th scope="row" valign="top"><?php esc_html_e('Toggle license status', $product->get_text_domain() ); ?></th>
			<td class="yoast-license-toggler">
				...
				<?php if( $this->license_is_valid() ) { ?>
					...
					<small><?php esc_html_e( '(deactivate your license so you can activate it on another WordPress site)', $product->get_text_domain() ); ?></small>
				<?php } else {
					...
					<?php } else {
						esc_html_e( 'Please enter a license key in the field below first.', $product->get_text_domain() );
					}
		...
		<tr valign="top">
			<th scope="row" valign="top"><?php esc_html_e( 'License Key', $product->get_text_domain() ); ?></th>
			<td>
				...
				<p class="help"><?php echo wp_kses_post(sprintf( __( "You defined your license key using the %s PHP constant.", $product->get_text_domain() ), '<code>' . $this->license_constant_name . '</code>' )); ?></p>
				<?php } ?>
			</td>
		</tr>
...
if( $this->license_is_valid() ) {
	...
	if( $expiry_date !== false ) {
		...
		echo wp_kses_post(sprintf( __( 'Your %s license will expire on %s.', $product->get_text_domain() ), $product->get_item_name(), date('F jS Y', $expiry_date ) ));

		if( strtotime( '+3 months' ) > $expiry_date ) {
			echo wp_kses_post(sprintf( ' ' . __('%sRenew your license now%s.', $product->get_text_domain() ), '<a href="'. $this->product->get_tracking_url( 'renewal' ) .'">', '</a>' ));
		}
