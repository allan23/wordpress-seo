...
class yoast_i18n {
	...
	private function promo_message() {
		...
		if ( $this->translation_exists && $this->translation_loaded && $this->percent_translated < 90 ) {
			$message = esc_html__( 'As you can see, there is a translation of this plugin in %1$s. This translation is currently %3$d%% complete. We need your help to make it complete and to fix any errors. Please register at %4$s to help complete the translation to %1$s!', $this->textdomain );
		} else if ( ! $this->translation_loaded && $this->translation_exists ) {
			$message = esc_html__( 'You\'re using WordPress in %1$s. While %2$s has been translated to %1$s for %3$d%%, it\'s not been shipped with the plugin yet. You can help! Register at %4$s to help complete the translation to %1$s!', $this->textdomain );
		} else if ( ! $this->translation_exists ) {
			$message = esc_htm__( 'You\'re using WordPress in a language we don\'t support yet. We\'d love for %2$s to be translated in that language too, but unfortunately, it isn\'t right now. You can change that! Register at %4$s to help translate it!', $this->textdomain );
		}
	...
	public function promo() {
		...
		if ( $message ) {
			...
			echo '<h2>' . sprintf( esc_html__( 'Translation of %s', $this->textdomain ), $this->plugin_name ) . '</h2>';
			...
			echo '<p>' . $message . '</p>';
			echo '<p><a href="' . $this->register_url . '">' . esc_html__( 'Register now &raquo;', $this->textdomain ) . '</a></p>';
			echo '</div>';
		}
	}
